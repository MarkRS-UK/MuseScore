...
void scriptHighlighter::colorScheme() {
      // default color scheme
      m_colors[scriptEdit::Normal]     = QColor("#000000");
      m_colors[scriptEdit::Comment]    = QColor("#808080");
      m_colors[scriptEdit::Number]     = QColor("#008000");
      m_colors[scriptEdit::String]     = QColor("#800000");
      m_colors[scriptEdit::Operator]   = QColor("#808000");
      m_colors[scriptEdit::Identifier] = QColor("#000020");
      m_colors[scriptEdit::Keyword]    = QColor("#000080");
      m_colors[scriptEdit::BuiltIn]    = QColor("#008080");
      m_colors[scriptEdit::Marker]     = QColor("#ffff00");
      }
...
void scriptHighlighter::setColor(scriptEdit::ColorComponent component, const QColor& color)
      {
...
void scriptHighlighter::highlightBlock(const QString& text)
            ...
            switch (state) {
                  case State::Start:
                        ...
                        if (ch.isSpace()) {
                              ...
                              i = text.length();
                              setFormat(start, text.length(), m_colors[scriptEdit::Comment]);
                              }
                        else if (ch == '/' && next != '*') {
                              ++i;
                              state = State::Regex;
                              ...
                              if (!QString("(){}[]").contains(ch))
                                    setFormat(start, 1, m_colors[scriptEdit::Operator]);
                              if (ch =='{' || ch == '}') {
                                    bracketPositions += i;
                                    if (ch == '{')
                                          bracketLevel++;
                  ...
                  case State::Number:
                        if (ch.isSpace() || !ch.isDigit()) {
                              setFormat(start, i - start, m_colors[scriptEdit::Number]);
                              state = State::Start;
                              }
                        else {
                              ++i;
                  ...
                  case State::Identifier:
                        if (ch.isSpace() || !(ch.isDigit() || ch.isLetter() || ch == '_')) {
                              ...
                              if (m_keywords.contains(token))
                                    setFormat(start, i - start, m_colors[scriptEdit::Keyword]);
                              else if (m_knownIds.contains(token))
                                    setFormat(start, i - start, m_colors[scriptEdit::BuiltIn]);
                              state = State::Start;
                              }
                        else {
                              ++i;
                  ...
                  case State::String:
                        if (ch == text.at(start)) {
                              ...
                              if (prev != '\\') {
                                    ++i;
                                    setFormat(start, i - start, m_colors[scriptEdit::String]);
                                    state = State::Start;
                                    }
                              else {
                                    ++i;
                  ...
                  case State::Comment:
                        if (ch == '*' && next == '/') {
                              ++i;
                              ++i;
                              setFormat(start, i - start, m_colors[scriptEdit::Comment]);
                              state = State::Start;
                              }
                        else {
                              ++i;
      ...
      if (!m_markString.isEmpty()) {
            ...
            markerFormat.setBackground(m_colors[scriptEdit::Marker]);
            markerFormat.setForeground(m_colors[scriptEdit::Normal]);
            for (;;) {
      ...
      if (!bracketPositions.isEmpty()) {
            scriptBlockData *blockData = reinterpret_cast<scriptBlockData*>(currentBlock().userData());
            if (!blockData) {
                  blockData = new scriptBlockData;
...
void scriptHighlighter::mark(const QString &str, Qt::CaseSensitivity caseSensitivity)
      {
      m_markString = str;
...
QStringList scriptHighlighter::keywords() const
      {
...
void scriptHighlighter::setKeywords(const QStringList &keywords)
      {
...
void scriptHighlighter::addKeywords(const char* keyWords[]) {
...
scriptEdit::scriptEdit(QWidget* parent) : QPlainTextEdit(parent) {
      ...
      setPalette(p);
      hl = new scriptHighlighter(document());
...
void scriptEdit::focusInEvent(QFocusEvent* event)
      {
...
void scriptEdit::focusOutEvent(QFocusEvent* event)
      {
...
void scriptEdit::move(QTextCursor::MoveOperation op)
      {
